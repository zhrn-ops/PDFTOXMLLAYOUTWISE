 ...
 */
export function observabilityUpdate(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => operations.UpdateObservabilityDestinationRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = encodeJSON("body", payload.UpdateObservabilityDestinationRequest, { explode: true });
    const pathParams = {
    ...
    const path = pathToFunc("/observability/destinations/{id}")(pathParams);
