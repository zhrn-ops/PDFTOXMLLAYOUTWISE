...
from pdf_to_jats.llm.openrouter_client import OpenRouterClient
...
from pdf_to_jats.models.document import DocumentZone
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        xml_btn = QPushButton("Generate JATS XML")
        ...
        actions.addWidget(unmerge_btn)
        actions.addWidget(redo_btn)
        actions.addWidget(xml_btn)
        ...
        viewer_column.addWidget(self.viewer)
        tree_column = QVBoxLayout()
        tree_column.addWidget(self.tree_hint)
        tree_column.addWidget(self.tree)
        tree_column.addWidget(self.selection_hint)
        tree_column.addWidget(self.selection_list)
        tree_column.addWidget(self.clear_selection_btn)
        body.addLayout(viewer_column, 3)
        body.addLayout(tree_column, 2)
        body.addWidget(self.props, 1)
    ...
    def generate_xml(self) -> None:
        ...
        output_path = self.config.generated_xml_dir / "article.xml"
        self.current_xml_path = self.generator.generate(self.document, output_path)
        errors = self.validator.validate(self.current_xml_path)
        if errors:
            for error in errors:
                self._log(f"Validation error: {error.message}")
        else:
            self._log(f"XML written to {self.current_xml_path}")
    ...
    def _update_document_model(self) -> None:
        ...
        author_blocks = [b for b in blocks if b.role == "author"]
    ...
    def _populate_tree(self) -> None:
        ...
        buckets: dict[str, list] = {"Title": [], "Authors": [], "Affiliations": [], "Abstract": []}
        for block in self.document.blocks:
            if block.role == "title":
    ...
    def _on_tree_item_changed(self, item, column: int) -> None:
        ...
        if block is None:
            return
        valid_roles = {"title", "author", "affiliation", "abstract"}
    ...
    def classify_selected_blocks(self) -> None:
        ...
        try:
            ...
            router_trace = "".join(traceback.format_exception(type(exc), exc, exc.__traceback__))
            self._log(f"OpenRouter classification failed:\n{router_trace}")
    ...
    def _selection_looks_like_paragraph_continuation(self, selected_blocks) -> tuple[bool, str]:
        ...
        blocks = self._sort_paragraph_continuation_blocks(selected_blocks)
        if len({block.page for block in blocks}) != 1:
            return False, "Select blocks from the same page only."
    ...
    def _sort_paragraph_continuation_blocks(self, blocks) -> list:
        ...
        widths = [float(block.width) for block in ordered if float(block.width) > 0]
        average_width = sum(widths) / len(widths) if widths else 0.0
        x_values = [float(block.x) for block in ordered]
        column_spread = max(x_values) - min(x_values)
        if average_width > 0 and column_spread > average_width * 0.65:
            return sorted(blocks, key=lambda block: (block.page, block.x, block.y))
        return ordered
